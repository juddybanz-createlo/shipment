...
export default function UserDashboard() {
  ...
  const availableShipments = shipments.filter(s => !s.claimedBy);
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-3xl font-display font-bold text-slate-900">Shipment Dashboard</h2>
          <p className="text-slate-500">Manage your consignments and discover new shipments to claim.</p>
        </div>
        <Button 
      ...
      <div className="flex border-b border-slate-200">
        <button 
          ...
          className={`px-6 py-3 text-sm font-medium transition-colors border-b-2 ${
            activeTab === 'my' ? 'border-brand-600 text-brand-600' : 'border-transparent text-slate-500 hover:text-slate-700'
          }`}
        >
        ...
        <button 
          ...
          className={`px-6 py-3 text-sm font-medium transition-colors border-b-2 ${
            activeTab === 'available' ? 'border-brand-600 text-brand-600' : 'border-transparent text-slate-500 hover:text-slate-700'
          }`}
        >
                ...
                <CardContent className="flex flex-col items-center text-center space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-slate-500 max-w-xs">
                      You haven't claimed any shipments yet. Enter a tracking code or check available shipments.
                    </p>
                  </div>
                  <Button variant="outline" onClick={() => setActiveTab('available')}>Browse available shipments</Button>
                </CardContent>
              </Card>
            ) : (
            ...
            {availableShipments.length === 0 ? (
              <Card className="border-dashed border-2 border-slate-200 bg-transparent py-20">
                <CardContent className="flex flex-col items-center text-center space-y-4">
                  ...
                  <div className="space-y-2">
                    <h3 className="text-xl font-display font-semibold text-slate-900">No available shipments</h3>
                    <p className="text-slate-500 max-w-xs">
                      All shipments have been claimed. Check back later for new consignments.
                    </p>
                  </div>
                </CardContent>
              </Card>
                    ...
                    <Card className="border-none shadow-lg shadow-slate-200/50 bg-white overflow-hidden group hover:shadow-xl transition-all">
                      <CardHeader className="bg-slate-50 p-6 border-b border-slate-100">
                        <div className="flex justify-between items-center">
                          <div className="space-y-1">
                            <p className="text-slate-400 text-[10px] font-bold uppercase tracking-widest">Tracking ID</p>
                            <h4 className="text-lg font-display font-bold text-slate-900">{shipment.trackingNumber}</h4>
                          </div>
                          <Badge variant="outline" className="border-brand-200 text-brand-700 bg-brand-50">Available</Badge>
                        </div>
                      </CardHeader>
                      <CardContent className="p-6 space-y-6">
                        <div className="flex items-center justify-between">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          <div className="p-3 bg-slate-50 rounded-lg">
                            ...
                            <p className="text-sm font-semibold text-slate-900">{shipment.weight || 'N/A'}</p>
                          </div>
                          <div className="p-3 bg-slate-50 rounded-lg">
                        ...
                        <Button 
                          ...
                          onClick={() => {
                            if (currentUser) {
                              claimShipment(shipment.trackingNumber, currentUser.email);
                              toast.success('Shipment claimed successfully!');
                            }
                          }}
                        >
